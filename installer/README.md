...
Windows Steam판 1.06 / build 18387525용입니다. Python 설치는 필요 없습니다.

1. 게임을 종료하고 이 ZIP의 압축을 풉니다. 내부 `highrook-ko-test.patch.zip`은 풀지 않습니다.
2. `install-steam.cmd`를 실행합니다.
...
자동 탐색이 실패하면 아래 경로를 Steam 라이브러리 경로로 바꾸세요. 복원할 때는 `Install`을 `Restore`로 바꿉니다.
